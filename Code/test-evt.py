...
def plot_image_grid(images, figsize=(15, 15)):
    ...
    plt.tight_layout()
    plt.savefig("./show_test1.png", dpi=300, bbox_inches='tight')
    plt.close(fig)
...
if __name__ == '__main__':
    ...
    validate_dataset = datasets.ImageFolder(root=os.path.join(data_root, "origin"),
    ...
    model.eval()
    target_layer = model.tokenizer.conv_layers[2][-1]
    ...
    for i, val_data in enumerate(validate_loader):
        ...
        cv2.imwrite('test1_{}.png'.format(i), superimposed_img, [int(cv2.IMWRITE_PNG_COMPRESSION), 9, int(cv2.IMWRITE_PNG_BILEVEL), 0])
        # 将结果保存到列表中
