...
class ConfusionMatrix(object):
    ...
    def plot(self):
        ...
        plt.tight_layout()
        plt.savefig('evt_eni.png', dpi=300)
        plt.show()
...
def plot_image_grid(images, figsize=(15, 15)):
    ...
    plt.tight_layout()
    plt.savefig("./show_test3.png", dpi=300, bbox_inches='tight')
    plt.close(fig)
...
if __name__ == '__main__':
    ...
    for i, val_data in enumerate(validate_loader):
        ...
        cv2.imwrite('test3_{}.png'.format(i), superimposed_img, [int(cv2.IMWRITE_PNG_COMPRESSION), 9, int(cv2.IMWRITE_PNG_BILEVEL), 0])
